<?php
	...
	$sql = "SELECT concat('name:',f.Group) as name, concat('y:', sum(fp.SalesAmount)*100/" . $tot_amount[0] .") as y, concat('drilldown:', f.Group) as drilldown
            FROM dimsalesterritory f
            JOIN factsales fp ON (f.TerritoryID = fp.TerritoryID)
            GROUP BY name
            ORDER BY y DESC"   ;         
            //echo $sql;
    ...
    $sql = "SELECT f.`Group` AS Groups, SUM(fp.SalesAmount) AS tot_kat
            FROM factsales fp
            JOIN dimsalesterritory f ON f.TerritoryID = fp.TerritoryID
            GROUP BY Groups;";
    ...
    $sql = "SELECT f.`Group` AS Groups, 
       f.TerritoryName AS TerritoryName, 
       SUM(fp.SalesAmount) AS pendapatan_kat
FROM dimsalesterritory f
JOIN factsales fp ON f.TerritoryID = fp.TerritoryID
GROUP BY f.`Group`, f.TerritoryName;";
    ...
    // Query to get sales by product category
    $sqlSalesByProduct = "SELECT dp.ProductCategory AS ProductCategory, SUM(fs.SalesAmount) AS TotalSales 
        FROM factsales fs
        JOIN dimproduct dp ON fs.ProductID = dp.ProductID 
        GROUP BY dp.ProductCategory";
    $resultSalesByProduct = $mysqli->query($sqlSalesByProduct);
    
    // Check if there are results
    if ($resultSalesByProduct && $resultSalesByProduct->num_rows > 0) {
        // Initialize arrays to hold categories and data for the chart
        $categories = [];
        $data = [];
        
        // Loop through the results
        while ($row = $resultSalesByProduct->fetch_assoc()) {
            ...
            $categories[] = $row['ProductCategory']; // Product names
            $data[] = (float) $row['TotalSales'];     // Total sales (converted to float)
        }
    ...
    $query = " SELECT 
        dp.EmployeeName AS EmployeeName, 
        dt.Tahun AS SalesYear, 
        SUM(fs.SalesAmount) AS TotalSales 
    FROM factsales fs
    JOIN dimemployee dp ON fs.EmployeeID = dp.EmployeeID
    JOIN dimtimeall dt ON fs.TimeID = dt.TimeID
    GROUP BY dp.EmployeeName, dt.Tahun";
...
foreach ($chartData as $year => $salesData) {
    foreach ($salesData as $employee => $sales) {
...
foreach ($years as $index => $year) {
  ...
  foreach ($employees as $employee) {
      $dataPoints[] = $chartData[$year][$employee] ?? 0;
  }
   ...
   $color = ($year == 2011) ? 'red' : (($year == 2012) ? 'blue' : (($year == 2013) ? 'green' : (($year == 2014) ? 'yellow' : 'gray')));
    ...
    //1. Total Customer
    $sql2 = "SELECT count(distinct CustomerName) as jml_cust from dimcustomer";
  ...
  <div class="content-wrapper">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Column Chart: Sales by Product</h3>
        </div>
        <div class="card-body">
    ...
    <div style="width: 80%; margin: auto;">
        <canvas id="salesChart"></canvas>
                      ...
                      <div>
                        <iframe name="mondrian" src="http://localhost:8080/mondrian/testpage.jsp?query=projectuas"  style="height:300px ;width:800px; border:none; text-align:center;"></iframe> 
                      </div>
                    </figure>
                    ...
                    Highcharts.chart('container', {
                        ...
                        title: {
                            text: 'Persentase Nilai Penjualan - Wilayah Geografis'
                        },
                        subtitle: {
                            text: 'Klik di potongan kue untuk melihat detil nilai penjualan berdasarkan negara'
                        },
    ...
    Highcharts.chart('bar-container', {
        ...
        xAxis: {
            ...
            title: {
                text: 'Products' // Set title for x-axis (Products)
            },
            labels: {
        ...
        yAxis: {
            title: {
                text: 'Total Sales' // Set title for y-axis (Total Sales)
            },
            labels: {
                format: '{value:,.0f}' // Format the labels on y-axis with commas
            },
            min: 0, // Minimum value for y-axis
            max: 100000000, // Maximum value for y-axis (set to a higher value, like 100 million)
            tickInterval: 10000000, // Interval between ticks (in this case, every 10 million)
        },
        plotOptions: {
            column: {
                dataLabels: {
        ...
        const salesChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Total Sales per Year by Employee'
                    }
                },
                scales: {
                    y: {
